...
class SymbolIndexBucket(
    val toplevels: AtomicTrieMap[String, Seq[
      (SourcePath, Option[Either[AbsolutePath, GlobalSymbolIndex.Module]])
    ]],
    val definitions: AtomicTrieMap[String, Seq[SymbolLocation]],
      ...
      case packageParts =>
        ...
        if (isTrivialToplevelSymbol0) Nil
          ...
          toplevels.updateWith(symbol) {
            case Some(acc) =>
              val elem = SourcePath(input.path) -> originOpt
              Some {
                if (acc.contains(elem)) acc
                else acc :+ elem
              }
            case None =>
              Some(Seq(SourcePath(input.path) -> originOpt))
          }
          List(symbol)
        }
    }
  ...
  def indexSource(
    ...
    val (doc, overrides, toplevelMembers) =
      mtags.extendedIndexing(input, dialectOpt)
    val sourceTopLevels =
  ...
  def addToplevelSymbol(
    ...
    if (source.isScalaScript || !isTrivialToplevelSymbol(path, toplevel)) {
      toplevels.updateWith(toplevel) {
        case Some(acc) =>
          val elem = source -> Some(Right(module))
          Some {
            if (acc.contains(elem)) acc
            else acc :+ elem
          }
        case None =>
          Some(Seq(source -> Some(Right(module))))
      }
    }
  }
  ...
  private def removeOldEntries(
      ...
      val elems =
        toplevels
          ...
          definitions
            ...
            .map(_.map(_.path))
            .getOrElse(Seq.empty)
      elems.filter(_.exists())
    }

    toplevels.updateWith(symbol.value) {
      case None => None
      case Some(acc) =>
        val updated = acc.filter(t => exists.contains(t._1))
    ...
    definitions.updateWith(symbol.value) {
      case None => None
      case Some(acc) =>
        val updated = acc.filter(loc => exists.contains(loc.path))
        if (updated.isEmpty) None
        else Some(updated)
    }
  }
