package scala.tools.nsc.classpath.metals

// vendoring this is actually not needed
// we only need the AggregateClassPath.*Entry methods here (that we added ourselves)
...
case class AggregateClassPath(aggregates: Seq[ClassPath]) extends ClassPath {
  ...
  private[this] val packageIndex
      : collection.mutable.Map[String, Seq[ClassPath]] =
  ...
  override def findClass(className: String): Option[ClassRepresentation] = {
    ...
    if (noByteCode) None
      ...
      def findEntry(isSource: Boolean): Option[ClassRepresentation] = {
        aggregatesForPackage(packageName).iterator
  ...
  override def hasPackage(pkg: PackageName) =
  ...
  override def list(inPackage: PackageName): ClassPathEntries = {
    ...
    val distinctPackages: Seq[PackageEntry] =
      if (packages == null) Nil
        ...
        unsafeWrapArray(
          packages.toArray(new Array[PackageEntry](packages.size()))
        )
